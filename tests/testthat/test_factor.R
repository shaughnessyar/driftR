...
test_that("input errors trigged - missing parameters", {
  expect_error(dr_factor(result, dateVar = foo, timeVar = Time, format = "MDY", keepDateTime = FALSE),
               "A new variable name must be specified for corrFactor")
  expect_error(dr_factor(result, corrFactor = factors, timeVar = bar, format = "MDY", keepDateTime = FALSE),
               "An existing variable with date data must be specified for dateVar")
  expect_error(dr_factor(result, corrFactor = factors, dateVar = Date, format = "MDY", keepDateTime = FALSE),
               "An existing variable with time data must be specified for timeVar")
  expect_error(dr_factor(result, corrFactor = factors, dateVar = Date, timeVar = Time, keepDateTime = FALSE),
               "A format - either MDY or YMD - must be specified")
})

test_that("input errors trigged - variables invalid", {
  expect_error(dr_factor(result, corrFactor = factors, dateVar = foo, timeVar = Time, format = "MDY", keepDateTime = FALSE),
               "Variable foo, given for dateVar, cannot be found in the given data frame")
  expect_error(dr_factor(result, corrFactor = factors, dateVar = Date, timeVar = bar, format = "MDY", keepDateTime = FALSE),
               "Variable bar, given for timeVar, cannot be found in the given data frame")
  expect_error(dr_factor(result, corrFactor = Date, dateVar = Date, timeVar = Time, format = "MDY", keepDateTime = FALSE),
...
test_that("input errors trigged - format invalid", {
  expect_error(dr_factor(result, corrFactor = factors, dateVar = Date, timeVar = Time, format = "foo", keepDateTime = FALSE),
...
result <- dr_factor(result, corrFactor = factors, dateVar = Date, timeVar = Time, format = "MDY", keepDateTime = FALSE)
...
result2 <- dr_factor(testData2, corrFactor = factors, dateVar = Date, timeVar = Time, format = "YMD", keepDateTime = FALSE)
result3 <- dr_factor(testData3, corrFactor = factors, dateVar = Date, timeVar = Time, format = "MDY", keepDateTime = FALSE)
